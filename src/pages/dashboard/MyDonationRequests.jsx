...
const MyDonationRequests = () => {
    ...
    return (
        <div>
            {requests.data ? <DonationRequestTable donations={requests.data} afterDelete={requests.refetch}></DonationRequestTable> : <Loading></Loading>}
        </div>
    );
};

export const DonationRequestTable = ({ donations, title = "My donation requests", afterTable = '', afterDelete = () => { } }) => {
                    ...
                    <tbody>
                        ...
                        {donations.map(item => <tr key={item._id}>
                            ...
                            <td className='flex gap-1'>
                                <Link className="btn btn-info" to={`/edit/donation/${item._id}`}>Edit</Link>
