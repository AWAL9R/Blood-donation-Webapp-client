...
const BloodDonationRequests = ({ limit=10, title, afterTable = null, route = 'pending-donation-requests', hiddenIfNoData = false }) => {
    ...
    const requests = useQuery({
        queryKey: [route, user?.email, limit, currentPage, status], queryFn: async () => {
            const data = await axiosSecure.get(`/${route}?${limit ? `limit=${limit}` : ''}&skip=${limit*currentPage}&status=${status}`)
        ...
        <div>
            {requests.data ? ((hiddenIfNoData && requests.data?.length > 0) || (hiddenIfNoData == false && requests.data)) ?
                <section className='p-4'>
                    <div className="container bg-base-100 rounded-2xl shadow p-6 md:p-8 my-10">
                    <div className="overflow-x-auto">
                        <DonationRequestTable shouldFilter={route!="pending-donation-requests"} status={status} setStatus={setStatus} donations={requests.data} afterDelete={requests.refetch} title={title || "All Pending donation requests"}></DonationRequestTable>
                    </div>
